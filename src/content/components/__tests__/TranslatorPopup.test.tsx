...
beforeAll(() => {
  ...
  global.window.translation = {
    canTranslate: mock(() =>
      Promise.resolve("readily" as TranslationAvailability),
    ),
  ...
  global.window.ai = {
    summarizer: {
      capabilities: mock(() =>
        Promise.resolve({ available: "readily" as const }),
      ),
      create: mock(() =>
        Promise.resolve({
          summarize: mock((text: string) =>
            Promise.resolve(`要約: ${text.substring(0, 20)}...`),
          ),
          destroy: mock(),
        }),
      ),
...
describe("TranslatorPopup", () => {
  ...
  test("should render with selected text", () => {
    const { getByText } = render(
      <TranslatorPopup selectedText="Hello world" onClose={() => {}} />,
    );
  ...
  test("should show preview for long text", () => {
    const longText =
      "This is a very long text that should be truncated in the preview section of the popup component";
    const { getByText } = render(
      <TranslatorPopup selectedText={longText} onClose={() => {}} />,
    );
  ...
  test("should call onClose when close button is clicked", () => {
    ...
    const { getByText } = render(
      <TranslatorPopup selectedText="Hello world" onClose={onCloseMock} />,
    );
  ...
  test("should have correct structure", () => {
    const { container } = render(
      <TranslatorPopup selectedText="Test text" onClose={() => {}} />,
    );
  ...
  test("should have action buttons with correct classes", () => {
    const { container } = render(
      <TranslatorPopup selectedText="Test" onClose={() => {}} />,
    );
